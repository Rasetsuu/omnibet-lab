...
def ids_are_unique(ids: list[str]) -> bool:
...
def build_report(root: Path) -> Dict[str, Any]:
    ...
    source_load_prefixes = contract.get("unique_html_id_prefixes", [])
    all_html_ids = html_ids(html)
    source_terminal_page_exists = contract.get("page_id") in html
    source_load_ids = [id_value for id_value in all_html_ids if any(id_value.startswith(prefix) for prefix in source_load_prefixes)]
    checks = {
        ...
        "html_button_ids_present": all(button_id in html for button_id in button_ids),
        "html_ids_unique": ids_are_unique(all_html_ids),
        "source_load_ids_unique": ids_are_unique(source_load_ids) and len(source_load_ids) == 4,
        "source_load_buttons_bound": contains_all(app_js, ["bindSourceLoadButton", "load-source-terminal-report", "load-source-terminal-report-topbar", "load-source-terminal-sample", "load-source-terminal-sample-topbar"]),
